...
func (ctl *budgetController) RegisterRoutes(app *fiber.App) {
	...
	budget.Get("/categories", handlers.SecureAuth(), ctl.GetCategories)
...
func (ctl *budgetController) GetCategories(ctx *fiber.Ctx) error {
	_, err := handlers.UserFromContext(ctx)
	if err != nil {
		return err
	}

	categories, err := ctl.budgetService.GetAllCategories()
	if err != nil {
		return ctx.JSON(types.GenericResponse{
			Success: false,
